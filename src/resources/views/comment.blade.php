        ...
        @if (Auth::id() === $comment['userId'])
            <div class="comment-content comment-content--right">
                ...
                <div class="comment-area comment-area--right">
                    ...
                    <form action="/item/comment/delete/{{ $item->id }}/{{ $comment['id'] }}" method="post">
                    @method('delete')
                    @csrf
                    <button class="submit-button__delete" type="submit" onclick="return confirm('コメントを削除しますか？')">コメント削除</button>
                    </form>
                </div>
            </div>
        @else
